...
//  BadgeComplete badgeCompleted(@Param("badgeName") String badgeName, @Param("userEmail") String userEmail);
  ...
  @Query(value="Match (a:`" + DataConstants.USER + "`)-[r:`" + DataConstants.HAS_COMPLETED_BADGE_LABEL + "`]-(b:`" + DataConstants.BADGE + "`) where id(a)={userId} return b",
    countQuery = "Match (a:`" + DataConstants.USER + "`)-[r:`" + DataConstants.HAS_COMPLETED_BADGE_LABEL + "`]-(b:`" + DataConstants.BADGE + "`) where id(a)={userId} return count(b)")
...
//  Badge checkBadgeCompleted(@Param("badgeName") String badgeName, @Param("userEmail") String userEmail);
  ...
  @Query(value="Match (u:`" + DataConstants.USER + "`),(t:`" + DataConstants.BADGE + "`) where id(u)={userId} and" +
    " not (u)-[:" + DataConstants.HAS_COMPLETED_BADGE_LABEL + "]-(t) return t",
  countQuery = "Match (u:`" + DataConstants.USER + "`),(t:`" + DataConstants.BADGE + "`) where id(u)={userId} and" +
    " not (u)-[:" + DataConstants.HAS_COMPLETED_BADGE_LABEL + "]-(t) return count(t)")
  Page<Badge> findRemainingBadges(@Param("userId") Long userId, Pageable pageable);

  Badge findByNameAndLevel(String name, Integer level, @Depth int i);
