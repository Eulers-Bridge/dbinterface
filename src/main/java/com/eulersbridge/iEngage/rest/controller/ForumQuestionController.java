...
public class ForumQuestionController {
  ...
  ResponseEntity<ForumQuestion> createForumQuestion(@RequestBody ForumQuestion forumQuestion) {
    if (LOG.isInfoEnabled())
      LOG.info("attempting to create forumQuestion " + forumQuestion);
  ...
  ResponseEntity<ForumQuestion> readForumQuestion(@PathVariable Long forumQuestionId) {
    if (LOG.isInfoEnabled())
      LOG.info(forumQuestionId + " attempting to get forumQuestion. ");
  ...
  ResponseEntity<ForumQuestion> updateForumQuestion(@PathVariable Long forumQuestionId, @RequestBody ForumQuestion forumQuestion) {
    if (LOG.isInfoEnabled())
      LOG.info("Attempting to update forumQuestion. " + forumQuestionId);
    ...
    if ((null != forumQuestionUpdatedEvent)) {
      if (LOG.isDebugEnabled())
        LOG.debug("forumQuestionUpdatedEvent - " + forumQuestionUpdatedEvent);
  ...
  ResponseEntity<Response> deleteForumQuestion(@PathVariable Long forumQuestionId) {
    if (LOG.isInfoEnabled())
      LOG.info("Attempting to delete forumQuestion. " + forumQuestionId);
  ...
  public @ResponseBody
  ResponseEntity<Response> like(@PathVariable Long forumQuestionId, @PathVariable String email) {
    if (LOG.isInfoEnabled())
      LOG.info("Attempting to have " + email + " like forumQuestion. " + forumQuestionId);
  ...
  public @ResponseBody
  ResponseEntity<Response> unlike(@PathVariable Long forumQuestionId, @PathVariable String email) {
    email = Util.getUserEmailFromSession();
    RequestHandledEvent result = likesService.unlike(email, forumQuestionId);
  ...
  ResponseEntity<Iterator<LikeInfo>> findLikes(
    ...
    pageLength = Integer.parseInt(pageSize);
    if (LOG.isInfoEnabled())
      LOG.info("Attempting to retrieve liked users from forumQuestion " + forumQuestionId + '.');
    Direction sortDirection = Direction.DESC;
    if (direction.equalsIgnoreCase("asc")) sortDirection = Direction.ASC;
