...
public class TicketEventHandler implements TicketService {
  ...
  public CreatedEvent createTicket(CreateTicketEvent createTicketEvent) {
    ...
    if (LOG.isDebugEnabled())
      LOG.debug("Finding election with nodeId = " + electionId);
    ...
    if (electionId != null) {
      ...
      if (elect != null) {
        ...
        ticket.setElection(elect.toNode());
  ...
  public UpdatedEvent updateTicket(UpdateTicketEvent updateTicketEvent) {
    ...
    if (ticketOld == null) {
      ...
      Ticket result = ticketRepository.save(ticket, 0);
      if (LOG.isDebugEnabled())
    ...
    if (candidates != null) {
      ...
      if (0 == dets.size()) {
        ...
        if ((null == ticket) ||
          ((null == ticket.getName()))) {
          if (LOG.isDebugEnabled())
            LOG.debug("Null or null properties returned by findOne(ticketId)");
          nare = AllReadEvent.notFound(null);
        } else {
