...
public class ConfigurationEventHandler implements ConfigurationService
{
			...
			CreateConfigurationEvent createConfigurationEvent)
	{
		...
		ConfigurationCreatedEvent configurationCreatedEvent = new ConfigurationCreatedEvent(
				result.getConfigId(), result.toConfigurationDetails());
			...
			UpdateConfigurationEvent updateConfigurationEvent)
	{
		...
		else
		{
			Configuration result = configurationRepository.save(configuration);
			if (LOG.isDebugEnabled())
				LOG.debug("updated successfully" + result.getConfigId());
			return new ConfigurationUpdatedEvent(result.getConfigId(),
			...
			DeleteConfigurationEvent deleteConfigurationEvent)
	{
		...
		else
		{
			...
			ConfigurationDeletedEvent configurationDeletedEvent = new ConfigurationDeletedEvent(
					configId);
