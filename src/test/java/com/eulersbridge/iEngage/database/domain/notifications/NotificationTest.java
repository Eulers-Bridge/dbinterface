...
public class NotificationTest {
  ...
  public final void testSetupForSaveUsingNodeId() {
    User testData = DatabaseDataFixture.populateUserGnewitt2();
  ...
  public final void testSetupForSaveUsingEmail() {
    ...
    hasNotification.setUser(saveData);
    notification.setHasNotificationRelationship(hasNotification);
    when(userRepository.findOne(anyLong())).thenReturn(testData);
  ...
  public final void testFromNotificationDetails() {
    ...
    assertEquals("", dets.getUserId(), notificationTest.hasNotificationRelationship.getUser().getNodeId());
  }
  ...
  /**
   * Test method for {@link com.eulersbridge.iEngage.database.domain.notifications.Notification()}.
   */
  @Test
  ...
  /**
   * Test method for {@link com.eulersbridge.iEngage.database.domain.notifications.Notification(com.eulersbridge.iEngage.database.domain.User)}.
   */
  @Test
  public final void testSetUser() {
    User user = DatabaseDataFixture.populateUserGnewitt();
    assertNotEquals("", notification.hasNotificationRelationship.getUser(), user);
    HasNotification hasNotification = new HasNotification();
    hasNotification.setUser(user);
    notification.setHasNotificationRelationship(hasNotification);
    assertEquals("", user, notification.getHasNotificationRelationship().getUser());
  }
