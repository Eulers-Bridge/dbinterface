...
//  private static Logger LOG = LoggerFactory.getLogger(ForumQuestionEventHandlerTest.class);
//
//  @Mock
...
//    when(fqRepository.findById(any(Long.class)).get()).thenReturn(testData);
...
//    Long nodeId = 1l;
//    when(fqRepository.findById(any(Long.class))).thenReturn(Optional.of(testData));
...
//    ForumQuestion testData = DatabaseDataFixture.populateForumQuestion1();
//    when(fqRepository.findById(any(Long.class)).get()).thenReturn(testData);
//    doNothing().when(fqRepository).deleteById((any(Long.class)));
...
//    if (LOG.isDebugEnabled()) LOG.debug("DeletingForumQuestion()");
//    ForumQuestion testData = DatabaseDataFixture.populateForumQuestion1();
//    when(fqRepository.findById(any(Long.class))).thenReturn(null);
//    doNothing().when(fqRepository).deleteById((any(Long.class)));
//    DeleteForumQuestionEvent deleteForumQuestionEvent = new DeleteForumQuestionEvent(testData.getNodeId());
//    DeletedEvent evtData = service.deleteForumQuestion(deleteForumQuestionEvent);
//    assertFalse(evtData.isEntityFound());
